...
const { semanticSearch } = require("../utils/semanticSearch"); // ✅ ADD THIS
          ...
          if (Array.isArray(metadata)) {
            ...
            metadata.forEach((item, index) => {
              try {
                ...
                console.error(
                  `❌ Error parsing file metadata item ${index}:`,
                  innerParseError
                );
            ...
            console.log(
              "✅ Processed files from metadata array:",
              files.length
            );
          }
...
exports.askChatbot = async (req, res) => {
  ...
  try {
    ...
    const finalMessages = buildAIMessagesWithSmartContext(
      ...
      contextType,
      urlContent,
      fileContextResult // ✅ ADD THIS PARAMETER
    );
...
async function getFileContextBasedOnUpload(
  ...
  try {
    console.log(`📄 Getting enhanced file context for conversation: ${conversation_id}`);
    
    // ✅ EARLY CHECK: If no files in conversation, return empty context
    ...
    if (fileCount[0]?.count === 0) {
      console.log(`📭 No files found in conversation ${conversation_id} - skipping RAG`);
      return {
        fileContext: "",
        fileNames: [],
        fileCount: 0,
    ...
    // ✅ GET ALL FILES FROM DATABASE FIRST
    const allFiles = await executeQuery(
      `SELECT file_path, extracted_text, file_metadata, created_at, status
       FROM uploaded_files 
       WHERE conversation_id = ? AND user_id = ? 
    ...
    console.log(`🔍 Found ${allFiles.length} total files in conversation ${conversation_id} for user ${user_id}`);

    // ✅ ENHANCED: Use semantic search for better context retrieval
    let semanticContext = "";
    let semanticChunks = [];
    ...
    if (allFiles.length > 0 && userMessage && userMessage.length > 10) {
      ...
      console.log(`🎯 Search context: User ${user_id}, Conversation ${conversation_id}`);
...
// console.log('🧩 Semantic Chunks for AI:', JSON.stringify(searchResult.chunks, null, 2));
        ...
        if (searchResult.hasResults) {
          ...
          console.log(`✅ Semantic search found ${searchResult.chunks.length} relevant chunks`);
          const filesInvolved = searchResult.chunks ? [...new Set(searchResult.chunks.map(c => c.fileName))] : [];
console.log(`📊 Files involved: ${filesInvolved.join(', ') || 'Unknown'}`);

        }
      } catch (semanticError) {
        console.log(`⚠️ Semantic search failed, falling back to traditional method:`, semanticError.message);
      }
    }
    ...
    // ✅ PRIORITY 1: Add semantic search results first if available
    if (semanticContext && semanticContext.length > 100) {
      fullContext += `🧠 SEMANTIC SEARCH RESULTS (Most Relevant Content):\n${semanticContext}\n\n${"=".repeat(80)}\n\n`;
      console.log(`✅ Added semantic context: ${semanticContext.length} characters`);
    }
    ...
    // ✅ PRIORITY 2: Add new upload if exists
    if (hasNewUpload) {
      fullContext += `🆕 NEWLY UPLOADED FILE (PRIORITY):\n${extracted_summary}\n\n${"=".repeat(
        60
      )}\n\n`;
    ...
    // ✅ PRIORITY 3: Add existing files (excluding the new one if it exists)
    if (allFiles && allFiles.length > 0) {
      allFiles.forEach((file, index) => {
        // Get file name
        let fileName = "Unknown File";
        ...
        if (
          ...
          // ✅ SKIP if content is already covered by semantic search (avoid duplication)
          ...
          if (isAlreadyInSemantic) {
            console.log(
              `⏭️ Skipped ${fileName} - already covered by semantic search`
            );
            return;
          }
          ...
          // ✅ Add as previous file context (truncated if too long)
          ...
          console.log(
            `✅ Added previous file: ${fileName} (${file.extracted_text.length} chars, truncated: ${truncatedText.length})`
          );
        } else {
          console.log(`❌ Skipped file: ${fileName} - invalid extracted text`);
        }
    ...
    console.log(`📄 ENHANCED FILE CONTEXT SUMMARY:`);
  ...
  chunkSummaries: semanticChunks.map(c => c.summary),
    ...
    console.error("❌ Error getting enhanced file context:", error);
    return {
      fileContext: "",
      fileNames: [],
      fileCount: 0,
...
function buildAIMessagesWithSmartContext(
  ...
  contextType = "none",
  urlContent = "",
  fileContextResult = null // ✅ ADD THIS PARAMETER
) {
...
const handleAIFileGeneration = async (
  ...
  console.log(
    `🔍 Raw response contains CREATE_FILE: ${aiResponse.includes(
      "[CREATE_FILE:"
    )}`
  );
  ...
  while (searchText.includes("[CREATE_FILE:")) {
    ...
    console.log(
      `🔍 Extracted full tag (${fullTag.length} chars): ${fullTag.substring(
        0,
        200
      )}...`
    );
  ...
  for (const fullTag of createFileMatches) {
    ...
    try {
      ...
      if (fileResult.success) {
        ...
        console.log(`✅ File created: ${fileResult.filename}`);
      ...
      cleanedResponse = cleanedResponse.replace(
        fullTag,
        `\n❌ **Could not create ${cleanFilename}.${
          fileType || "docx"
        }**\n*Error: ${error.message}*\n`
      );
...
// 🤖 GENERATE COMPREHENSIVE SUMMARY OF CONVERSATION - Async function
